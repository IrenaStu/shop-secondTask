...
public class WearHouseService {
    ...
    public void reduceStock(Product product, int quantity) {
        wearHouseStock.getStockMap().compute(product, (p, q) -> {
            if (q == null) {
                throw new RuntimeException("Product " + p.getProductName() + " not found in warehouse");
            }
            if (q < quantity) {
                throw new RuntimeException("Not enough stock for product " + p.getProductName() +
                        ": requested " + quantity + ", available " + q);
            }
